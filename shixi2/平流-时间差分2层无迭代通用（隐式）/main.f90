...
!!!!!!!!!!!!!!!!!!!!!!!!!!
   module differencemethod   !!!封装模块，包含空间前差、空间后差、空间中央差、时间隐式格式子程序
     implicit none
     private
     save
	 ...
	 real, parameter :: A1=0.5		 ! now timepoint
	 real, parameter :: B1=0.5 		 ! next	timepoint  A1+B1=1.0
	 real, parameter :: Pi=3.1415926
	 public :: FXzhong,matrix,mean1,mean2
	 public :: Pi,nx,dt,nt,C,dx,A1,B1
	 ...
	 subroutine matrix(a,b,c,r,x)    !!!时间隐式格式子程序
	   implicit none
	   real, intent(in)  :: a(Nx)
	   real, intent(in)  :: b(Nx)
	   real, intent(in)  :: c(Nx)
	   ...
	   real aa(Nx),bb(Nx),cc(Nx),rr(Nx),bbN(Nx),rrN(Nx)
	   integer i
	   bb(1)=b(1)
	   aa(1)=a(1)
	   cc(1)=c(Nx)
	   ...
	   rrN(1)=r(Nx)
	   do i=1,Nx-2
	      bb(i+1)=b(i+1)-a(i+1)*c(i)/bb(i)
		  aa(i+1)=-a(i+1)*aa(i)/bb(i)
		  cc(i+1)=-c(i)*cc(i)/bb(i)
		  rr(i+1)=r(i+1)-a(i+1)*rr(i)/bb(i)
		  bbN(i+1)=bbN(i)-aa(i)*cc(i)/bb(i)
		  rrN(i+1)=rrN(i)-rr(i)*cc(i)/bb(i)
	   enddo  	   
	   bbN(Nx)=bbN(Nx-1)-(aa(Nx-1)+c(Nx-1))*(a(NX)+cc(Nx-1))/bb(Nx-1)
	   rrN(Nx)=rrN(Nx-1)-rr(Nx-1)*(a(Nx)+cc(Nx-1))/bb(Nx-1)
	   x(Nx)=rrN(Nx)/bbN(Nx)
	   do i=Nx,2,-1
	      x(i-1)=(rr(i-1)-c(i-1)*x(i)-aa(i-1)*x(Nx))/bb(i-1)
	   enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
    program main     !!!!主程序
	   use differencemethod,only:nx,dt,nt,C,dx,A1,B1,Pi, &
	                             FXzhong,matrix,mean1,mean2    !!!!!调用上面differencemethod模块中的部分子程序和数组
       implicit none
       real Xinit(1:nx)     !!!!定义输入数组
	   real Xnow(1:nx),Xnext(1:nx)
	   real Xdt(1:nx)
       real Xoutput(1:nx,0:5)     !!!!定义输出数组(共6列，第一列为初始值，第二列为解析解，第三-六列为四个时间积分时刻的数值解)
	   real Xmean,Xmean1(0:5),Xmean2(0:5)
	   real xa(1:nx),xb(1:nx),xc(1:nx),xr(1:nx)
	   real kk           !-c*dt/(4*dx)    
	   ...
	   !!!采用空间中央差、时间隐式格式求取数值解，并给输出数组的第三-六列赋值数值解
	   ! a b c based on Xspace difference method
	   !  implicit T X difference method (Xn+1-Xn)/dt=A*FXzhong(Xn)+B*FXzhong(Xn+1))
	   kk=-B1*c*dt/(2*dx)
	   xa(1:nx)=kk
	   xb(1:nx)=1.0
	   xc(1:nx)=-kk
       ...
       do t=1,nt
                       call 	FXzhong(Xnow,Xdt)   !!!空间中央差格式
		  xr=Xnow+Xdt*dt*A1         
		  call   matrix(xa,xb,xc,xr,Xnext) !!!时间隐式格式
          Xnow=Xnext
		  do i=1,nx
	   ...
	   write(22, '(a4,f4.1,a4,f4.1,a4,f4.1,a4,f4.1,a4,f4.1a4,I4)') &
	        	"dx=",dx,"C=",C,"dt=",dt,"A1=",A1,"B1=",B1,"nt=",nt
       do i=1,nx
